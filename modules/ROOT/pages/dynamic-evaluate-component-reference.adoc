...
この Core コンポーネントは、結果が別のスクリプトになる式を評価し、次にそのスクリプトを評価して最終的な結果を得ます。この動作により、xref:transform-component-about.adoc[Transform Message コンポーネント]を使用してハードコード化する代わりに、動的にスクリプトを選択できます。
