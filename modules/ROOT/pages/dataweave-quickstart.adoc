...
+
DataWeave では、多くの出力形式と入力形式がサポートされます。準備が整ったら、xref:dataweave-formats.adoc[DataWeave でサポートされる形式]についての詳細を確認してください。
+
. <<step_3>>に進みます。
...
----
+
この例のプレビューで問題が発生した場合は、`myRead.car` を `myRead."car"` に変更してください。準備が整ったら、xref:dw-core-functions-read.adoc[read] 関数、xref:dataweave-formats.adoc[DataWeave でサポートされる形式]、xref:dataweave-selectors.adoc[DataWeave セレクタ]についての詳細を確認してください。
+
. 出力をプレビューします。
+
[source,json,linenums]
...
NOTE: `payload` を使用して入力データを参照する例 (xref:dw-core-functions-contains.adoc#contains1[contains] や xref:dw-core-functions-mapobject.adoc[mapObject] 関数の例など) の場合、<<provide_payload>> で説明されている技法を使用してペイロード関連の問題を回避できます。
...
Core コンポーネントが*「fx」*項目で DataWeave セレクタと短い DataWeave 式を受け入れます。
+
