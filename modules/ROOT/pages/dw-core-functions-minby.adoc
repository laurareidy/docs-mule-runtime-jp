...
|===
| Name (名前)   | 説明
| `item` | 入力配列内の要素 (型が `Number` (数値)、`Boolean` (ブール)、`DateTime` (日時)、`LocalDateTime` (ローカル日時)、`Date` (日付)、`LocalTime` (ローカル時刻)、`Time` (時刻)、`TimeZone` (タイムゾーン) のいずれか)。`&#36;` で参照できます。
|===

=== 例

次の例では、配列のオブジェクト (キー-値ペア) 内の最小数値を返します。`item.a` を使用してオブジェクトの値を選択しています。`[ { "a" : 1 }, { "a" : 3 }, { "a" : 2 } ] minBy &#36;.a` のように、匿名パラメータを使用して同じ式を記述することもできます。
