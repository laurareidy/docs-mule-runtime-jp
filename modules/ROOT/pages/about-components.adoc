...
コネクタとモジュールでの操作と同様に、Core コンポーネントは
Mule アプリケーションのフローの重要なビルディングブロックです。Core コンポーネントは、Mule イベントが
...
+
image::components-core-studio.png[Studio での Core コンポーネント]
+
...
+
image::components-core-fd.png[Design Center での Core コンポーネント]
+
Design Center では、次に説明されている Core コンポーネントの多くが提供されます。Design Center UI は
...
処理するフローおよびメッセージプロセッサに関する情報を収集。 
xref:business-events.adoc[「ビジネスイベント」]も参照してください。
...
Mule フローで処理を開始 (またはトリガ) するコンポーネントが含まれます。 
xref:scheduler-concept.adoc[Scheduler] はエンドポイントです。設定可能な間隔で開始する
フローをトリガします。
...
スコープは、イベントプロセッサのシーケンス (他の Core コンポーネント、
...
補正ロジックを適用できます。 
xref:try-scope-concept.adoc[「Try スコープ」]を参照してください。
...
各行を処理するために使用できます。 
xref:for-each-scope-concept.adoc[「For Each スコープ」]を参照してください。
