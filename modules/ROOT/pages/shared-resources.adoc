...
. <<Referencing Shared Resources, >>アプリケーションで<<Referencing Shared Resources,共有リソースを参照します>>。
. <<Deploying with Shared Resources, >>ドメインとアプリケーションを共有リソースと共に<<Deploying with Shared Resources,デプロイします>>。ドメインとアプリケーションは<<Deploying Domain Bundles,ドメインバンドル>>で一緒にデプロイすることもできます。
...
次の例では、`mule-domain-config.xml` で HTTP コネクタを共有リソースとして定義しています。
...
ドメインに関連付けられているすべての Mule アプリケーションは、プロジェクト自身の中でリソースを参照するように、設定内でリソースを参照することによって共有リソースを利用できます。次の例では、HTTP リスナコネクタが `HTTP_Listener_Configuration` という名前の共有リソースを参照しています。
