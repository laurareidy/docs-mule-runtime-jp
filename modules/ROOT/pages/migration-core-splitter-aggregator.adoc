...
+
もう 1 つフローを作成し、`aggregator:aggregator-listener` をソースとして追加します。すべての集約 (完了してもしなくても) によってそのリスナがトリガされます。次に、集約は `vm:publish` 操作によってメインフローに送信されます。
+
...
xref:for-each-scope-xml-reference.adoc[For Each スコープリファレンス]