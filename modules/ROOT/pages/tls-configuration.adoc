...
+
. プロンプトに対して他の値を入力します。任意の値を入力してください。応答例:
+
...
+
. 次のプロンプトに応答します。RETURN キーを押すと同じパスワードが使用されます。
+
...
----
+
. プロンプトに対して、証明書に関連付けるキーストア用にセットアップしてあるパスワードを入力します。
+
Enter keystore password (キーストアのパスワードを入力してください): *mule123*
...
+
. プロンプトに対して、キーストア用にセットアップしてあるパスワードを入力します。
+
...
* <<XML の編集による TLS の設定,XMLファイルを直接編集します。>>
* <<Anypoint Studio を使用して TLS を設定する,Anypoint Studio 7 を使用します。>>
* <<Design Center を使用して TLS を設定す,Design Center フローデザイナを使用します。>>
