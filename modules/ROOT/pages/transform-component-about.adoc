...
Transform (または Transform Message) コンポーネントは、入力データを新しい出力構造または形式に変換します。コンポーネントのビジュアルマッパーを使用して受信したデータ構造の要素をドラッグアンドドロップすることで変換を暗黙的に構築するか、または DataWeave 式言語で変換スクリプトを明示的に作成できます。
