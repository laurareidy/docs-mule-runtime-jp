...
JMS トランスポートでは、JMS メッセージの本文を含めるためにペイロードを必要とし、Mule のアウトバウンドプロパティを使用して JMS プロパティとヘッダーをカスタマイズしていました。新しい Mule 4 アプローチでは、JMS の 'publish' 操作で、入力パラメータのみを使用してパブリッシュする JMS メッセージを完全に構築します。
