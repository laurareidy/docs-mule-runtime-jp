...
ソースや操作から新しいメッセージが生成されると、*メッセージの両方の部分 (ペイロードと属性) が新しい値に置換され*、以前の属性は失われます。操作の呼び出しで属性からの情報を保持する必要がある場合は、以前の属性を変数に保存するか、または xref:target-variables.adoc[呼び出された操作に `target` パラメータを設定] できます。
